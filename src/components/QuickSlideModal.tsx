...
export default function QuickSlideModal({ isOpen, onClose, onSuccess }: QuickSlideModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/slides/quick-slide', {
        method: 'POST',
        headers: {
      ...
      if (data.status === 'success') {
        ...
        setError(data.message || 'Failed to create quick slide');
      }
    } catch (err) {
      console.error('Error creating quick slide:', err);
      setError('Failed to create quick slide. Please try again.');
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold">Create Quick Slide</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {showSuccess && (
              ...
              >
                Quick slide created successfully! Refreshing...
              </div>
            )}
          ...
          >
            ...
            >
              {isSubmitting ? 'Creating...' : 'Create Quick Slide'}
            </button>
          </div>
        </form>
      </div>
