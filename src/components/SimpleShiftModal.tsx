...
export default function SimpleShiftModal({ isOpen, onClose, onSuccess }: SimpleShiftModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/slides/simple-shift-slide', {
        method: 'POST',
        headers: {
      ...
      if (data.status === 'success') {
        ...
        setError(data.message || 'Failed to create simple shift slide');
      }
    } catch (err) {
      console.error('Error creating simple shift slide:', err);
      setError('Failed to create simple shift slide. Please try again.');
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold">Create Simple Shift Slide</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {showSuccess && (
              ...
              >
                Simple shift slide created successfully! Refreshing...
              </div>
            )}
          ...
          >
            ...
            >
              {isSubmitting ? 'Creating...' : 'Create Simple Shift Slide'}
            </button>
          </div>
        </form>
      </div>
